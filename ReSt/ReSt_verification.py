...
reward_model.eval()
responses = ResponseCollector(out_path="ReSt/results/MathVista_temp3.json")
...
for data in dataset_json:
    ...
    if not os.path.isfile(image_path):
        continue
    for i in range(N):
        prompt, image = two_shots_prompt_building_single_image(input, image_path, processor, hint=data['hint'], type=data['type'], add=ReSt)
        response = generate_response(processor, model, prompt, image, temperature=0.3, do_sample=True)
        ...
        while flag:
            ...
            step_prompt, image = two_shots_prompt_building_single_image(input, image_path, processor, hint=data['hint'], type=data['type'], add=response_step)
            if response_step == "":
                flag = False
            else:
