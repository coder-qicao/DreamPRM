...
reward_model.eval()
responses = ResponseCollector(out_path="ReSt/results/ScienceQA.json")
...
for data in dataset_json:
    ...
    if not os.path.isfile(image_path):
        continue
    for i in range(N):
        prompt, image = two_shots_prompt_building_single_image(input, image_path, processor, add=ReSt)
        response = generate_response(processor, model, prompt, image, temperature=0.1+0.2*i, do_sample=True)
        true_false = verify_answer(response, data['ground_truth'], 'multi_choice')
        print(f"{data['id']}:{true_false}")
        responses.add_response(ReSt+response, data['input'], data['id'], true_false)
        # find steps with the highest score
        max_score = -10
        flag = True if i != N-1 else False
        index = 1
        while flag:
            response_step = split_step(index, ReSt + response)
            step_prompt, image = two_shots_prompt_building_single_image(input, image_path, processor, add=response_step)
            if response_step == "":
                flag = False
            else:
